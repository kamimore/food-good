...
<div class="row">
	...
	<div class="col-md-6 col-lg-3 grid-margin stretch-card">
	  <div class="card">
		<div class="card-body">
		  <h1 class="font-weight-light mb-4">
			<?php
			$roww=mysqli_query($conn,"SELECT count(order_detail.dish_details_id) as t,
      dish.dish from order_detail,dish_details,dish WHERE order_detail.dish_details_id=dish_details.id and
      dish_details.dish_id=dish.id group by order_detail.dish_details_id order by count(order_detail.dish_details_id) desc limit 1");
			$row=mysqli_fetch_assoc($roww);
	...
	<div class="col-md-6 col-lg-3 grid-margin stretch-card">
	  <div class="card">
		<div class="card-body">
		  <h1 class="font-weight-light mb-4">
			<?php
			$user=mysqli_query($conn,"select count(order_master.user_id)
			 as t,user.name from order_master,user WHERE order_master.user_id=user.id GROUP 
			 BY order_master.user_id order by count(order_master.user_id) desc limit 1");
  ...
  $sql="select order_master.*,order_status.order_status as order_status_str from order_master,order_status
   where order_master.order_status=order_status.id order by order_master.id desc limit 5";
		  ...
		  <div class="table-responsive">
			<table class="table table-hover">
			  ...
			  <tbody>
						...
						<tr>
                            <td>
								<div class="div_order_id">
									<a href="order_detail?id=<?php echo $row['id']?>"><?php echo $row['id']?></a>
								</div>
							</td>
                            <td>
						...
						<tr>
							<td colspan="6" style="text-align:center;text-decoration:underline;font-weight:bold;">No data available in table</td>
						</tr>
						<?php } ?>
                      </tbody>
			</table>
