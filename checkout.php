...
if(isset($_POST['place_order'])){
	if($cart_min_price!=''){
		if($totalCartPrice>=$cart_min_price){
		}else{
			$is_error='yes';
		}
	}
...
if($is_error==''){
	...
	$checkout_name=safe($_POST['checkout_name']);
	$checkout_email=safe($_POST['checkout_email']);
	$checkout_mobile=safe($_POST['checkout_mobile']);
	$checkout_zip=safe($_POST['checkout_zip']);
	$checkout_address=safe($_POST['checkout_address']);
	$payment_type=safe($_POST['payment_type']);

	if(isset($_SESSION['COUPON_CODE']) && isset($_SESSION['FINAL_PRICE'])){
		$coupon_code=safe($_SESSION['COUPON_CODE']);
		$final_price=safe($_SESSION['FINAL_PRICE']);
	}else{
		$coupon_code='';
   ...
   $sql="insert into
   order_master
   (user_id,name,email,mobile,address,zipcode,total_price,
   order_status,payment_status,added_on,coupon_code,final_price,payment_type,cancel_by)
   values('".$_SESSION['FOOD_USER_ID']."','$checkout_name','$checkout_email'
   ,'$checkout_mobile','$checkout_address','$checkout_zip','
   $totalCartPrice','1','pending','$added_on','$coupon_code','$final_price','$payment_type','none')";
  ...
  mysqli_query($conn,$sql);
	...
	foreach($cartArr as $key=>$val){
		mysqli_query($conn,"insert into order_detail(order_id,dish_details_id,price,qty)
    values('$insert_id','$key','".$val['price']."','".$val['qty']."')");
	}
//emptyCart();
  ...
  if($payment_type=='wallet'){
    ...
    mysqli_query($conn,"update order_master set payment_status='success' where id='$insert_id'");
    $emailHTML=orderEmail($insert_id);
    include('smtp/PHPMailerAutoload.php');
    send_email($email,$emailHTML,'Order Placed');
    //emptyCart();
  ...
  if($payment_type=='paytm'){
    ////////////////////
    $foodid=$_SESSION['FOOD_USER_ID'];
    $orderid=$_SESSION['ORDER_ID'];
   // mysqli_query($conn,"INSERT INTO `userorderids` (`userid`, `orderid`) VALUES ('$foodid', '$orderid')");
    ///////////////////
    $paytm_oid=$insert_id.'_'.$_SESSION['FOOD_USER_ID'].'_'.rand(00000,11111);
    $html='<form method="post" action="pgRedirect" name="frmPayment" style="display:none;">
        <input id="ORDER_ID" tabindex="1" maxlength="20" size="20"
            ...
            <input id="CUST_ID" tabindex="2" maxlength="12" size="12"
             name="CUST_ID" autocomplete="off" value="'.$_SESSION['FOOD_USER_ID'].'"><input id="INDUSTRY_TYPE_ID"
              tabindex="4" maxlength="12" size="12" name="INDUSTRY_TYPE_ID" autocomplete="off" value="Retail">
              <input id="CHANNEL_ID" tabindex="4" maxlength="12" size="12" name="CHANNEL_ID" autocomplete="off"
              value="WEB"><input title="TXN_AMOUNT" tabindex="10"
              type="text" name="TXN_AMOUNT"
              value="'.$final_price.'">
              <input value="CheckOut" type="submit"	onclick=""></td>
              </form>
              <script type="text/javascript">document.frmPayment.submit();
                ...
                <div class="row">
                    <div class="col-lg-9">
                        <div class="checkout-wrapper">
                                ...
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h5 class="panel-title"><span>2.</span> <a data-toggle="collapse"
										 data-parent="#faq" href="#payment-2">Other information</a></h5>
                                    </div>
													...
													<div class="row">
														...
														<div class="col-lg-3 col-md-6">
															<div class="billing-info">
																...
																<input type="text"  name="checkout_zip" value=" " required>
															</div>
														</div>
														<div class="col-lg-12 col-md-12">
															<div class="billing-info">
																<label>Address</label>
																<input type="text"  name="checkout_address" value=" " required>
															</div>
														</div>
														...
														<div class="col-lg-5 col-md-12">
															<div class="billing-back-btn">
																<div class="billing-btn">
																	<button type="button" name="place_order" id="couponItem"
                            ...
                            $is_dis='';
                            $low_msg='';
                            if($getWalletAmt>=$totalCartPrice){
                        ...
                        <div class="checkout-progress">
                            <div class="shopping-cart-content-box">
								...
								<ul>
									...
									<li class="single-shopping-cart">
										<div class="shopping-cart-img">
											<a href="#"><img alt="" src="<?php echo load.$list['image']?>"></a>
										</div>
										<div class="shopping-cart-title">
											...
											<span><?php echo
														(int)$list['qty']*(int)$list['price'];?> Rs</span>
										</div>
								...
								<div class="shopping-cart-total">
									<h4>Total : <span class="shop-total"><?php echo $totalCartPrice?> Rs</span></h4>
								</div>
