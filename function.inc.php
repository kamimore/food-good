...
{
  global $conn;
  $fast=mysqli_real_escape_string($conn,$run);
  return $fast;
}
...
function send_email($email,$html,$subject){
	...
	$mail->SMTPSecure="tls";
	$mail->SMTPAuth=true;
	$mail->Username="negivipin996@gmail.com";
  $mail->Password="xxxxxxxxxxxxxxx";
	$mail->SetFrom("negivipin996@gmail.com");
...
//checcccccccccccccccccccccccccccccccccccccckkkkkkkkkkkkkk
...
function manageUserCart($uid,$qty,$attr)
{
  ...
  $res=mysqli_query($conn,"select * from dish_cart where user_id='$uid' and dish_detail_id='$attr'");
   if(mysqli_num_rows($res) > 0){
     $row=mysqli_fetch_assoc($res);
     $cid=$row['id'];
     mysqli_query($conn,"update dish_cart set
     qty='$qty' where id='$cid'");
   }
   else {
     $added_on=date('Y-m-d h:i:s');

     mysqli_query($conn,"insert into
      dish_cart(user_id,dish_detail_id,qty,added_on)
...
function getDishCartStatus(){
    ...
    dish_details.status,dish.status as dish_status,dish.id
...
$row=mysqli_fetch_assoc($res);
		if($row['dish_status']==0){
			$id=$row['id'];
			$res=mysqli_query($conn,"select id from dish_details where
      dish_id='$id'");
  ...
  $cartArr=array();
    ...
    foreach($getUserCart as $list)
    {
      ...
      $getDishDetail=getDishDetailById($list['dish_detail_id']);
            ...
            //$cartArr[$list['dish_detail_id']]['price']=getDishPriceById($list['dish_detail_id']);
...
/*{
  global $conn;
  $res=mysqli_query($conn,"select * from dish_details where id='$id'");
  if(mysqli_num_rows($res) > 0){
    $row=mysqli_fetch_assoc($res);
    return $row['price'];
...
function getDishDetailById($id){
	...
	global $conn;
	$res=mysqli_query($conn,"select dish.dish,dish.image,dish_details.price from dish_details,dish where
	dish_details.id='$id' and dish.id=dish_details.dish_id");
...
function removeDishFromCartByid($id){
	if(isset($_SESSION['FOOD_USER_ID'])){
		global $conn;
		$res=mysqli_query($conn,"delete from dish_cart where dish_detail_id='$id' and user_id=".$_SESSION['FOOD_USER_ID']);
	}else{
...
function getUserDetailsByid($uid=''){
  ...
  $data=array();
	...
	if(isset($_SESSION['FOOD_USER_ID']) or $uid > 0){
  
    
		 $uid=isset($_SESSION['FOOD_USER_ID'])?$_SESSION['FOOD_USER_ID']:$uid;
    

	$row=mysqli_fetch_assoc(mysqli_query($conn,"select * from
  user where id='$uid'"));
	$data['name']=$row['name'];
...
function emptyCart(){
	if(isset($_SESSION['FOOD_USER_ID'])){
		global $conn;
		$res=mysqli_query($conn,"delete from dish_cart where user_id=".$_SESSION['FOOD_USER_ID']);
	}else{
		unset($_SESSION['cart']);
	}
}
...
function getcartTotalPrice(){
	...
	foreach($cartArr as $list){
		$totalPrice=$totalPrice+((int)$list['qty']*(int)$list['price']);
	}
	return $totalPrice;
}
...
<html>
  ...
  <body>
    <!--<span class="preheader">This is an invoice for your purchase on <a href="'.user_login.'">Vayuananda_website</a></span>-->
          ...
          <table class="email-content" width="100%" cellpadding="0" cellspacing="0" role="presentation">
            ...
            <tr>
              ...
              <strong>Order ID:</strong> '.$order_id.'
                        ...
                        about this invoice, simply reply to
                        this email or reach out to our <a href="'.user_login.'">support team</a> for help.</p>
                        <p>Cheers,
                          <br><a href="user_login">vayuananda</a></p>
                        <!-- Sub copy -->
...
function manageWallet($uid,$amt,$type,$msg,$payment_id=''){
	global $conn;
	$added_on=date('Y-m-d h:i:s');
	$sql="insert into wallet(user_id,amt,msg,type,added_on,payment_id)
   values('$uid','$amt','$msg','$type','$added_on','$payment_id')";
	$res=mysqli_query($conn,$sql);
}
